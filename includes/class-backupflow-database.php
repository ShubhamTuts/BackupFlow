...
class BackupFlow_Database {
	...
	private function query_import_statement( $statement ) {
		...
		$wpdb->last_error = '';
		$result           = $wpdb->query( $statement ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Restore intentionally executes SQL generated by BackupFlow.

		if ( false !== $result ) {
			return array( 'foreign_key_fallback' => false );
		}

		$error = $wpdb->last_error;
		if ( $this->is_foreign_key_create_error( $statement, $error ) ) {
